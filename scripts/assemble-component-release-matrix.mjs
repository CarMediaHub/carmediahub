...
export function assembleComponentReleaseMatrix(input) {
  ...
  validateComponentReleaseMatrix(matrix);
  return matrix;
}
...
function parse(args) {
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    if (!["--component-id", "--version", "--platforms", "--release-dir", "--output"].includes(key) || value === undefined || value.startsWith("--")) fail("usage: assemble-component-release-matrix --component-id <id> --version <semver> --platforms <comma-separated-platforms> --release-dir <signed-json-dir> --output <matrix.json>");
    values.set(key, value);
  }
  return { componentId: values.get("--component-id"), version: values.get("--version"), platforms: values.get("--platforms")?.split(","), releaseDir: values.get("--release-dir"), output: values.get("--output") };
}
...
if (process.argv[1] !== undefined && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    const matrix = assembleComponentReleaseMatrix({ ...input, releases });
