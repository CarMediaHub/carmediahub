...
function fixture() {
  ...
  for (const relative of ["dist/cli.js", "public/admin/index.html", "scripts/upgrade-preflight.mjs", "scripts/native-install-plan.mjs", "scripts/native-windows-service.mjs", "scripts/native-linux-service.mjs", "scripts/validate-native-bundle.mjs", "config/components.json", "config/components.schema.json", "config/core.schema.json", "config/core.example.json", "node_modules/fastify/package.json", "node_modules/@fastify/cookie/package.json", "node_modules/pg/package.json", "node_modules/@carmediahub/sdk/package.json", "node_modules/@carmediahub/sdk/dist/index.js"]) { const location = path.join(bundleRoot, relative); fs.mkdirSync(path.dirname(location), { recursive: true }); fs.writeFileSync(location, "{}\n"); }
...
test("creates a Windows install plan without executing service registration", () => { const input = fixture(); const plan = createNativeInstallPlan({ ...common(input), platform: "windows" }, { statfs: () => ({ bavail: 4, bsize: 1024 }) }); assert.equal(plan.platform, "windows"); assert.equal(plan.bundle.databaseSchemaVersion, 1); assert.match(plan.service.command, /dist\\cli\.js/iu); assert.equal(plan.resources.freeBytes, 4096); });
test("creates a Linux install plan with hardened unit text", () => { const input = fixture(); const plan = createNativeInstallPlan({ ...common(input), platform: "linux", nodePath: "/opt/cmh/runtime/node", serviceName: "carmediahub-core" }, { checkResources: () => ({ bundleRoot: "/opt/carmediahub", configPath: "/etc/carmediahub/core.json", dataDir: "/var/lib/carmediahub", freeBytes: 4096, requiredFreeBytes: 1 }) }); assert.match(plan.service.unitText, /NoNewPrivileges=true/iu); assert.match(plan.service.unitText, /ReadWritePaths/iu); });
