...
export function validateDeployment(compose, dockerfile) {
  ...
  if (!/groupadd\s+--system.*carmediahub/u.test(dockerfile) || !/useradd\s+--system.*carmediahub/u.test(dockerfile) || !/USER\s+carmediahub/u.test(dockerfile)) fail("Docker image must run as the dedicated non-root Core user");
  if (!/COPY\s+carmediahub\/config\/components\.json\s+carmediahub\/config\/components\.schema\.json\s+carmediahub\/config\/component-release\.schema\.json\s+carmediahub\/config\/core\.schema\.json\s+carmediahub\/config\/core\.example\.json/u.test(dockerfile) || !/COPY --from=build\s+\/workspace\/carmediahub\/config\/components\.json\s+\/workspace\/carmediahub\/config\/components\.schema\.json\s+\/workspace\/carmediahub\/config\/component-release\.schema\.json\s+\/workspace\/carmediahub\/config\/core\.schema\.json\s+\/workspace\/carmediahub\/config\/core\.example\.json/u.test(dockerfile)) fail("Docker image must copy only controlled configuration artifacts");
