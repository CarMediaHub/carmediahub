...
export function validateDeployment(compose, dockerfile) {
  ...
  if (!/COPY\s+carmediahub\/config\/components\.json\s+carmediahub\/config\/components\.schema\.json\s+carmediahub\/config\/core\.example\.json/u.test(dockerfile) || !/COPY --from=build\s+\/workspace\/carmediahub\/config\/components\.json\s+\/workspace\/carmediahub\/config\/components\.schema\.json\s+\/workspace\/carmediahub\/config\/core\.example\.json/u.test(dockerfile)) fail("Docker image must copy only controlled configuration artifacts");
