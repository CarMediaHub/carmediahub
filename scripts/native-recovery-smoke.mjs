...
const bundleRoot = parse(process.argv.slice(2).filter((value) => value !== "--"));
...
try {
  ...
  console.log(JSON.stringify({ bundleFiles: result.bundleRoot === bundleRoot, restored: true, currentSchemaVersion: result.currentSchemaVersion, targetSchemaVersion: result.targetSchemaVersion }));
} catch (error) {
