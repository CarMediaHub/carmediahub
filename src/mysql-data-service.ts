...
export async function exportMysqlPluginData(client: MysqlQueryClient, scope: ScopeContext): Promise<PluginDataExport> {
  ...
  for (const row of rows) { const records = grouped.get(row.collection) ?? []; records.push({ key: row.record_key, value: jsonValue(row.value_json), updatedAt: String(row.updated_at) }); grouped.set(row.collection, records); }
  ...
  const result: PluginDataExport = { exportedAt: new Date().toISOString(), scope: { organizationId, userId, installationId }, migrations: migrations.map((row) => ({ version: row.version, name: row.name, appliedAt: String(row.applied_at) })), collections: [...grouped.entries()].map(([name, records]) => ({ name, records })) };
