...
test("browser worker driver launches only the verified browser-engine component", async () => {
  ...
  let captured: { executablePath?: string; env?: Record<string, string>; args?: readonly string[]; userDataDir?: string; acceptDownloads?: boolean; permissions?: readonly string[]; serviceWorkers?: string } | undefined;
  let closed = false;
  let routed = false;
  let unrouted = false;
  ...
  try {
    ...
    const handle = await startBrowserWorker({
      ...
      targetRegistry,
      targetId: "media",
      runtime: { launchPersistentContext: async (userDataDir, options) => { captured = { executablePath: options.executablePath, env: options.env, args: options.args, userDataDir, acceptDownloads: options.acceptDownloads, permissions: options.permissions, serviceWorkers: options.serviceWorkers }; return fakeContext; } }
    });
