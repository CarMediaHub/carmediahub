...
const scope: RuntimeCredentialScope = {
  deploymentId: "dep_real", organizationId: "org_real", userId: "user_real", deviceId: "device_real", sessionId: "session_real", installationId: "plugin_enabled", locale: "en", timeZone: "UTC", theme: "system", density: "comfortable", entry: "key", display: { deviceClass: "vehicle", input: ["touch", "remote"], fullscreenAvailable: true, viewport: { width: 1920, height: 1200 } }, policyVersion: 7
};
...
test("Broker broadcasts persisted context changes to the matching user Worker", async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "cmh-broker-"));
  const broker = new RuntimeBroker({ dataDir, installationEnabled: (id) => id === scope.installationId });
  try {
    const endpoint = await broker.start();
    ...
    await send(client.socket, client.decoder, request("hello", "broker.hello"));
    await send(client.socket, client.decoder, request("prove", "worker.prove", scope.installationId, { runtimeCredential: credential }));
    const notification = new Promise<RpcRequest>((resolve) => client.socket.once("data", (chunk: Buffer) => resolve(client.decoder.push(chunk)[0] as RpcRequest)));
    broker.broadcastContext(scope.userId, { locale: "ko", theme: "dark" });
    const message = await notification;
    assert.equal(message.method, "context.changed");
    const changed = (message.params as { context: { scope: { userId: string }; locale: string; theme: string; policyVersion: number } }).context;
    assert.equal(changed.scope.userId, scope.userId);
    assert.deepEqual({ locale: changed.locale, theme: changed.theme, policyVersion: changed.policyVersion }, { locale: "ko", theme: "dark", policyVersion: 8 });
    client.socket.end();
  } finally {
    await broker.stop();
    fs.rmSync(dataDir, { recursive: true, force: true });
    ...
    const workerRequest = new Promise<void>((resolve, reject) => {
      client.socket.once("data", (chunk: Buffer) => {
        try {
          ...
          assert.deepEqual(message.params, { method: "GET", path: "/media", headers: { "x-test": "yes" }, context: { locale: "en", timeZone: "UTC", theme: "system", density: "comfortable", entry: "key", display: { deviceClass: "vehicle", input: ["touch", "remote"], fullscreenAvailable: true, viewport: { width: 1920, height: 1200 } }, policyVersion: 7 } });
...
test("SDK worker client completes the broker handshake and serves a logical route", async () => {
  ...
  try {
    ...
    assert.deepEqual(worker.context, { scope: { deploymentId: scope.deploymentId, organizationId: scope.organizationId, userId: scope.userId, deviceId: scope.deviceId, sessionId: scope.sessionId, installationId: scope.installationId }, locale: "en", timeZone: "UTC", theme: "system", density: "comfortable", entry: "key", display: { deviceClass: "vehicle", input: ["touch", "remote"], fullscreenAvailable: true, viewport: { width: 1920, height: 1200 } }, policyVersion: 7 });
