...
test("managed browser resolver refuses missing capability or unhealthy components", async () => {
  ...
  const unhealthy = { pluginHasCapability: () => true, componentById: () => ({ id: "chromium", version: "1.0.0", executable: "chromium/1.0.0/chromium", checksum: "a".repeat(64), health: "unhealthy", verified: true }) } as unknown as Repository;
...
test("managed browser resolver refuses a browser component for another platform", async () => {
  const registry = new BrowserTargetRegistry(); registry.register({ id: "fixture", origins: ["https://fixture.example"] });
  const repository = { pluginHasCapability: () => true, componentById: () => ({ id: "chromium", version: "1.0.0", executable: "chromium/1.0.0/chromium", checksum: "a".repeat(64), health: "healthy", verified: true }) } as unknown as Repository;
