...
export class BrowserTaskExecutor {
  ...
  register(kind: BrowserTaskKind, handler: BrowserTaskHandler): void { if (this.handlers.has(kind)) throw new Error("Browser task handler is already registered"); this.handlers.set(kind, handler); }
