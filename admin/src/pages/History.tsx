...
export default function History() {
  ...
  const clear = async () => { const response = await fetch("/api/history", { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify(category === undefined ? {} : { category }) }); if (!response.ok) { message.error("Unable to clear history"); return; } message.success(category === undefined ? "History cleared" : "Category cleared"); refresh(); };
  return <ProLayout title="CarMediaHub" logo={false} route={{ routes }} location={{ pathname: "/admin/history" }} menuItemRender={(item, dom) => <a href={item.path}>{dom}</a>} actionsRender={() => [<Button key="logout" icon={<LogoutOutlined />} onClick={async () => { await fetch("/api/auth/logout", { method: "POST" }); window.location.href = "/admin/login"; }}>Sign out</Button>]}> 
    <ProCard title="Browsing history" extra={<Space><Input allowClear placeholder="Search title or route" value={keyword} onChange={(event) => setKeyword(event.target.value)} onPressEnter={() => refresh(1)} /><Select allowClear placeholder="Category" value={category} onChange={setCategory} options={[...new Set(entries.map((entry) => entry.category).filter((value): value is string => value !== undefined))].map((value) => ({ label: value, value }))} /><Button onClick={() => refresh(1)}>Search</Button><Popconfirm title={category === undefined ? "Clear all history?" : `Clear ${category} history?`} onConfirm={clear}><Button danger icon={<DeleteOutlined />}>Clear</Button></Popconfirm></Space>} style={{ margin: 24 }}><ProTable<HistoryEntry> rowKey="id" search={false} options={false} pagination={{ current: page, pageSize, total, showSizeChanger: false, onChange: (nextPage) => refresh(nextPage) }} dataSource={entries} columns={[{ title: "Title", dataIndex: "title" }, { title: "Plugin", dataIndex: "pluginId" }, { title: "Category", dataIndex: "category", render: (value) => value === undefined ? <Tag>Uncategorized</Tag> : <Tag color="blue">{value}</Tag> }, { title: "Route", dataIndex: "route" }, { title: "Device", dataIndex: "sourceDevice" }, { title: "Visited", dataIndex: "visitedAt" }]} /></ProCard>
  </ProLayout>;
}
