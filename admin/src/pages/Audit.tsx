...
export default function Audit() {
  ...
  return <ProLayout title="CarMediaHub" logo={false} route={{ routes: routes.map((item) => ({ ...item, name: item.path === "/admin/overview" ? t("nav.overview") : item.path === "/admin/audit" ? t("nav.audit") : item.path === "/admin/components" ? t("nav.components") : item.path === "/admin/media" ? t("nav.media") : item.path === "/admin/keys" ? t("nav.keys") : item.path === "/admin/security" ? t("nav.security") : t("nav.users") })) }} location={{ pathname: "/admin/audit" }} menuItemRender={(item, dom) => <a href={item.path}>{dom}</a>} actionsRender={() => [<Button key="logout" onClick={async () => { await fetch("/api/auth/logout", { method: "POST" }); window.location.href = "/admin/login"; }}>{t("nav.signOut")}</Button>]}> 
