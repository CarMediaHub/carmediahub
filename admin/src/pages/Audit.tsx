...
export default function Audit() {
  ...
  const refresh = async () => { const params = new URLSearchParams({ limit: String(limit), ...(type.trim() === "" ? {} : { type: type.trim() }), ...(keyword.trim() === "" ? {} : { keyword: keyword.trim() }) }); const response = await fetch(`/api/audit?${params.toString()}`); if (!response.ok) { message.error("Unable to load audit events"); return; } setEvents((await response.json()).events ?? []); };
  ...
  return <ProLayout title="CarMediaHub" logo={false} route={{ routes }} location={{ pathname: "/admin/audit" }} menuItemRender={(item, dom) => <a href={item.path}>{dom}</a>} actionsRender={() => [<Button key="logout" onClick={async () => { await fetch("/api/auth/logout", { method: "POST" }); window.location.href = "/admin/login"; }}>Sign out</Button>]}> 
    <ProCard title="Audit events" extra={<><Input placeholder="Action type" value={type} onChange={(event) => setType(event.target.value)} style={{ width: 160, marginRight: 8 }} /><Input placeholder="Keyword" value={keyword} onChange={(event) => setKeyword(event.target.value)} style={{ width: 160, marginRight: 8 }} /><InputNumber min={1} max={500} value={limit} onChange={(value) => { if (value !== null) setLimit(value); }} style={{ marginRight: 8 }} /><Button onClick={() => void refresh()}>Refresh</Button></>} style={{ margin: 24 }}><ProTable<AuditEvent> rowKey="id" search={false} options={false} dataSource={events} columns={[{ title: "Time", dataIndex: "createdAt" }, { title: "Action", dataIndex: "type" }, { title: "Actor", dataIndex: "actorId", render: (value) => value ?? "System" }, { title: "Subject", dataIndex: "subject" }]} /></ProCard>
  </ProLayout>;
}
