...
import { ProCard, ProForm, ProFormSelect, ProFormTextArea, ProTable } from "@ant-design/pro-components";
...
export default function Plugins() {
  ...
  const installSignedPackage = async (values: Record<string, unknown>): Promise<boolean> => {
    try {
      const release = JSON.parse(String(values.release));
      const response = await fetch("/api/plugins/packages/install", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(release) });
      if (!response.ok) { message.error(t("plugins.packageRejected")); return false; }
  ...
  const upgradePackage = async (values: Record<string, unknown>): Promise<boolean> => {
    try {
      const installationId = String(values.installationId);
      const release = JSON.parse(String(values.release));
      const response = await fetch(`/api/plugins/${encodeURIComponent(installationId)}/upgrade`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(release) });
      if (!response.ok) { message.error(t("plugins.upgradeRejected")); return false; }
  ...
  return <AdminShell pathname="/admin/plugins">
    <ProCard title={t("plugins.installPackage")} style={{ margin: 24 }}><ProForm onFinish={installSignedPackage}><ProFormTextArea name="release" label={t("plugins.packageRelease")} fieldProps={{ autoSize: { minRows: 8, maxRows: 20 } }} rules={[{ required: true }]} /></ProForm></ProCard>
    <ProCard title={t("plugins.upgradePackage")} style={{ margin: 24 }}><ProForm onFinish={upgradePackage}><ProFormSelect name="installationId" label={t("plugins.installation")} options={installations.filter((item) => item.status === "installed").map((item) => ({ label: `${item.packageId} ${item.packageVersion} (${item.id})`, value: item.id }))} rules={[{ required: true }]} /><ProFormTextArea name="release" label={t("plugins.upgradeRelease")} fieldProps={{ autoSize: { minRows: 8, maxRows: 20 } }} rules={[{ required: true }]} /></ProForm></ProCard>
    <ProCard title={t("plugins.registerManifest")} style={{ margin: 24 }}><ProForm onFinish={async (values) => { try { const manifest = JSON.parse(String(values.manifest)); const response = await fetch("/api/plugins", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(manifest) }); if (!response.ok) { message.error(t("plugins.manifestRejected")); return false; } message.success(t("plugins.installSuccess")); refresh(); return true; } catch { message.error(t("plugins.invalidJson")); return false; } }}><ProFormTextArea name="manifest" label={t("plugins.manifest")} fieldProps={{ autoSize: { minRows: 6, maxRows: 16 } }} rules={[{ required: true }]} /></ProForm></ProCard>
