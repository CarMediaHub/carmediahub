...
export default function Notifications() {
  ...
  const refresh = () => { void fetch("/api/notifications?limit=500").then((response) => response.ok ? response.json() : { notifications: [] }).then((data) => setItems(data.notifications ?? [])); };
  useEffect(refresh, []);
