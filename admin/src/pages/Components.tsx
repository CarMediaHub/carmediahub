...
async function validateBindingEndpoint(_rule: unknown, value: string, t: (key: "common.invalidEndpoint" | "common.endpointRestrictions") => string): Promise<void> {
  if (value === undefined || value.trim() === "") return;
  let endpoint: URL;
  try { endpoint = new URL(value); } catch { throw new Error(t("common.invalidEndpoint")); }
  if ((endpoint.protocol !== "http:" && endpoint.protocol !== "https:") || endpoint.username !== "" || endpoint.password !== "" || endpoint.search !== "" || endpoint.hash !== "" || endpoint.hostname === "" || (endpoint.port !== "" && (!/^\d+$/u.test(endpoint.port) || Number(endpoint.port) < 1 || Number(endpoint.port) > 65535))) throw new Error(t("common.endpointRestrictions"));
}
