...
export default function Components() {
  ...
  return <ProLayout title="CarMediaHub" logo={false} route={{ routes: [{ path: "/admin/overview", name: "Overview", icon: <AppstoreOutlined /> }, { path: "/admin/components", name: "Components", icon: <CloudServerOutlined /> }, { path: "/admin/media", name: "Media roots", icon: <FolderOpenOutlined /> }, { path: "/admin/keys", name: "Entry keys", icon: <KeyOutlined /> }, { path: "/admin/security", name: "Security", icon: <SafetyCertificateOutlined /> }, { path: "/admin/users", name: "Users", icon: <TeamOutlined /> }] }} location={{ pathname: "/admin/components" }} menuItemRender={(item, dom) => <a href={item.path}>{dom}</a>} actionsRender={() => [<Button key="logout" icon={<LogoutOutlined />} onClick={async () => { await fetch("/api/auth/logout", { method: "POST" }); window.location.href = "/admin/login"; }}>Sign out</Button>]}> 
    <ProCard title="Installed components" style={{ margin: 24 }}><ProTable<Component> rowKey="id" search={false} options={false} dataSource={components} columns={[{ title: "Component", dataIndex: "id" }, { title: "Version", dataIndex: "version" }, { title: "Executable", dataIndex: "executable" }, { title: "Health", dataIndex: "health", render: (value) => <Tag color={value === "healthy" ? "green" : "default"}>{value}</Tag> }, { title: "Action", render: (_, row) => <Button onClick={async () => { const response = await fetch(`/api/components/${row.id}/health`, { method: "POST" }); if (!response.ok) { message.error("Unable to check component"); return; } const result = await response.json() as { component: { health: string } }; message.success(`Component is ${result.component.health}`); refresh(); }}>Check</Button> }]} /></ProCard>
    <ProCard title="Component catalog" style={{ margin: 24 }}><ProTable<CatalogComponent> rowKey="id" search={false} options={false} dataSource={catalog} columns={[{ title: "Component", dataIndex: "displayName" }, { title: "Kind", dataIndex: "kind" }, { title: "Catalog version", dataIndex: "version" }, { title: "Status", dataIndex: "status", render: (value) => <Tag color={value === "installed" ? "green" : "default"}>{value}</Tag> }, { title: "Supported platforms", dataIndex: "platforms", render: (_, row) => row.platforms.join(", ") }]} /></ProCard>
