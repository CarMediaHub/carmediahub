...
서명 단계는 릴리스 운영자 워크스테이션에서 명시적으로 실행합니다.
...
CLI는 Ed25519 키와 `release.keyId`에 일치하는 공개 키 지문을 요구하고, 기존 출력 파일을 거부하며, 개인 키를 staging 또는 런타임 번들에 복사하지 않습니다. 전체 플랫폼 매트릭스는 `node scripts/validate-component-release-matrix.mjs --matrix <matrix.json>`로 검증할 수 있습니다.

플랫폼별 서명이 끝나면 명시한 플랫폼 목록으로 매트릭스를 조립합니다.
...
현재 Compose 프로필은 Core와 관리되는 SQLite 데이터 볼륨만 실행합니다. Core에는 플러그인 논리 데이터용 명시적 PostgreSQL 및 MySQL adapter가 있지만, 완전한 대체 백엔드 배포 프로필은 아직 이 Compose 파일에 포함되지 않았으며 대상 환경에서 명시적으로 구성하고 smoke 테스트해야 합니다.
...
Docker에서는 non-root Core 컨테이너가 호스트 bind mount에 직접 스냅샷을 쓰게 하지 말고, 컨테이너가 이미 초기화한 데이터 디렉터리에 복구하지 마세요. 운영자가 관리하는 쓰기 가능한 staging 위치에서 스냅샷을 만든 뒤 내보내고, 새 빈 디렉터리 또는 데이터 볼륨에 복구한 다음 검증 후 Core를 시작하며 원래 볼륨은 롤백용으로 보존해야 합니다.

명시적 `docker:backup-migrate` 도우미는 Node와 Docker CLI가 같은 운영 환경에 있어야 합니다. Docker 실행 파일의 절대 경로를 전달해야 하며, 이미 존재하는 복구 데이터 볼륨을 거부하고 격리된 컨테이너 staging 경로를 사용합니다.
...
`native-install-verify`는 읽기 전용입니다. 명시적 apply 후 계획된 서비스 계정, 정확한 Linux systemd 유닛 또는 Windows 서비스 구성, 활성화/실행 상태를 검사합니다. 검사 실패는 배포 드리프트이며 기존 서비스를 자동으로 덮어쓰지 않습니다.
